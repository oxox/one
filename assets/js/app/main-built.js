 ...
 * @version 1.0.0
 */
define('jquery.modernloading',['jquery','rAF'],function($){
 ...
 * @version 1.0.0
 */
define('jquery.modernui',['jquery','jquery.modernloading'],function($){
 ...
 * @author levinhuang
 */
define('../app/base',['jquery','./tray'],function($,T){
    ...
    //app.ini
    p.parseIniData = function(){
        var initJson = pub.session['ini'];
        if (initJson) {
            pub.iniData=JSON.parse(initJson);
            return;
        };
        if (!pub.fs.existsSync(pub.initFile)) {
            pub.iniData={
                "language":"zh-CN",
                "defaultLanguage":"en-US"
            };
            pub.fs.mkdirsSync(pub.dataRoot);
            pub.fs.writeJsonSync(pub.initFile,pub.iniData);
            pub.session['ini']=JSON.stringify(pub.iniData);
            return;
        };

        pub.iniData = pub.fs.readJsonSync(pub.initFile);
        pub.session['ini']=JSON.stringify(pub.iniData);
    };
        ...
        pub.fs.readdir(dirObj.path,function(err,files){
            ...
            for (var i = 0; i <len2; i++) {
                ...
                tempFile={
                    ...
                    'id':pub.generateFileIdByFilePath(dirObj.path+files[i]),
                    'name':files[i],
                    'ext':pub.path.extname(files[i]).replace('.','').toLowerCase(),
                    'dir':dirObj.path,
                    'isImg':pub.isImg(files[i]),
                ...
                tempFile.urlRoot = pub.getFileRootPath(tempFile.url);
...
define('../app/data',['jquery','./base'],function($,B){
    var p = {},pub={};
 ...
 */
define('../app/view',['jquery','./base'],function($,B){
...
*/
define('jquery.removeClassByPrefix',['jquery'],function($){
...
define('../app/themepicker',['jquery','./base','./view','jquery.removeClassByPrefix'],function($,B,V){
